...
def mount_mcp(
    ...
    *,
    ...
    the Model Context Protocol from your own app::
        ...
        mount_mcp(app, MyAgent, sessions=router.sessions, path="/agent/mcp")

    Args:
        app (FastAPI): The FastAPI application to mount on.
        agent_class (type[BaseAgent]): The agent class being served.
        name (Optional[str]): MCP server name. Defaults to the class name.
        version (str): MCP server version.
        model (Optional[str]): Default model used when this function creates its
            own session manager. Ignored if ``sessions`` is provided.
