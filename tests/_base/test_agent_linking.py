...
class TestAgentLinkingExecution:
    ...
    async def test_linked_agent_call_processing(self):
        ...
        with patch.object(HelperAgent, "__call__", new=AsyncMock(return_value=mock_response)) as mocked_call:
            helper = HelperAgent(model="test-model", api_key="test-key")
            main = MainAgent(model="test-model", api_key="test-key", helper=helper)
            ...
            assert response == mock_response  # or: assert response.final_output == "Helper completed the task"
...
class TestAgentLinkingEdgeCases:
    ...
    def test_circular_agent_references_forward_ref_not_supported(self):
        ...
        assert any("Forward reference not supported" in m and "agent_b" in m for m in msgs), (
    ...
    def test_deeply_nested_agent_hierarchies(self):
        ...
        assert (Level1Agent.__linked_agents__["specialist"] == 
                Level2Agent.__linked_agents__["specialist"] ==
                Level3Agent.__linked_agents__["specialist"] == SpecialistAgent)
